...
ASSISTment_evaluate_result_tryloss = 'ASSISTment_results_tryloss.csv'
fname_TrainData = ASSISTment_train
fname_TestData = ASSISTment_test
fname_MapData = ASSISTment_Category_MappingFile
fname_Result = ASSISTment_evaluate_result_tryloss
'''
PKU_evaluate_result_tryloss = '../data/PKU_MOOC/PKU_results_tryloss.csv'
fname_TrainData = PKU_train
fname_TestData = PKU_test
fname_MapData = PKU_Category_MappingFile
fname_Result = PKU_evaluate_result_tryloss
'''
...
def main(args):
    ...
    assert args.granularity_outmode in ['mean', 'max'], 'Unrecognized loss function for granularity output, mean or max?'
    multi_granined_outmode = args.granularity_outmode
    ...
    train_batches = BatchGenerator(train_response_list, batch_size, id_encoding, n_id, n_id, n_categories, random_embedding=random_embedding, skill_to_category_dict=skill2category_map, multi_granined_out=multi_granined_out,multi_granined_outmode=multi_granined_outmode)
    test_batches = BatchGenerator(test_response_list, batch_size, id_encoding, n_id, n_id, n_categories, random_embedding=random_embedding, skill_to_category_dict=skill2category_map, multi_granined_out=multi_granined_out,multi_granined_outmode=multi_granined_outmode)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--output-loss-mode',     
                        dest='granularity_outmode',
                        default="max",
                        type=str,
                        help="When using multi-grained information for output, use mean or max as loss function?")
